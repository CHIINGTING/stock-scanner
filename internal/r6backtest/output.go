...
func csvRow(t Trade) []string {
	return []string{
		...
		f(t.Exit5dReturn), f(t.Exit10dReturn), f(t.Exit20dReturn), f(t.Exit60dReturn),
...
func ComputeStats(name string, bucket int, trades []Trade, horizons []int, p Params) SetupStat {
	...
	retOf := func(t Trade, h int) float64 {
		switch h {
		case 5:
			return t.Exit5dReturn
		case 10:
			return t.Exit10dReturn
		case 20:
			return t.Exit20dReturn
		case 60:
			return t.Exit60dReturn
		}
		return math.NaN()
	}
	for _, h := range horizons {
		...
		if len(rs) > 0 {
			st.WinRate[h] = float64(wins) / float64(len(rs)) * 100
			st.AvgReturn[h] = mean(rs)
			st.MedianReturn[h] = median(rs)
		} else {
			st.WinRate[h], st.AvgReturn[h], st.MedianReturn[h] = math.NaN(), math.NaN(), math.NaN()
		}
	}
	var dds []float64
	stops := 0
...
func WriteMarkdown(path, title string, meta []string, stats []SetupStat, horizons []int) error {
	...
	for _, s := range stats {
		...
		b.WriteString("## " + head + "\n\n")
		b.WriteString(fmt.Sprintf("- sample_count: %d　confidence: %s\n", s.SampleCount, s.Confidence))
		...
		b.WriteString("\n| horizon | win_rate | avg_return | median_return |\n|---|---|---|---|\n")
		for _, h := range horizons {
			b.WriteString(fmt.Sprintf("| %dd | %s | %s | %s |\n", h,
				pct(s.WinRate[h]), pct(s.AvgReturn[h]), pct(s.MedianReturn[h])))
		}
		b.WriteString(fmt.Sprintf("\n- max_drawdown_avg: %s　max_drawdown_p90: %s　stop_hit_rate: %s\n",
			pct(s.MaxDrawdownAvg), pct(s.MaxDrawdownP90), pct(s.StopHitRate)))
